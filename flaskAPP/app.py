...
def plot_city(city, vehicles, passengers):
   # plt.cla()  # Clear the current axes
    ...
    plt.draw()
  #  plt.pause(5)  # Pause to allow time for the plot to update
...
def save_plot(city, vehicles, passengers, step):
    ...
    plt.close()
    # Introduce a pause between steps to allow time for the plot to update
    time.sleep(4)
    return filename
...
def simulate():
    if request.method == 'POST':
        # Handle the form submission and run the simulation
        ...
        # Pass the simulation parameters to the template
...
def run_simulation_step(data):
    ...
    for step in range(num_steps):
        ...
        # Save the plot for the current step
        plot_filename = save_plot(city, vehicles, passengers, step)

        # Send the plot filename to the client
        emit('update_plot', {
    'plot_filename': plot_filename,
    'foot_steps': [passenger.foot_steps for passenger in passengers],
    'vehicle_steps': [passenger.vehicle_steps for passenger in passengers]
})
